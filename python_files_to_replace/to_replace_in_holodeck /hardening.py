...
class Hard_CDM():
    ...
    def __init__(self, rho_s_3d_array, rs_3d_array, gamma_sp, sepa_init=1.0e3*PC, num_steps=300):
        self._num_steps = num_steps
        self._sepa_init = sepa_init
         # Ensure the arrays are writable by creating copies
        self._rho_s_3d_array = np.copy(rho_s_3d_array)
        self._rs_3d_array = np.copy(rs_3d_array)
        self._gamma_sp = gamma_sp

        return

class Hard_SIDM():

    def __init__(self, rt, r_sp_a4, v0_a0, vt, r_values_a0, r_values_a4, rho_core_values_a0, rho_core_values_a4, N1_array, N2_array, sepa_init=1.0e3*PC, num_steps=300):
        self._num_steps = num_steps
        self._sepa_init = sepa_init
        # Ensure the arrays are writable by creating copies
        self._rt = np.copy(rt)
        self._r_sp_a4 = np.copy(r_sp_a4)
        self._v0_a0 = np.copy(v0_a0)
        self._vt = vt # cgs
        self._r_values_a0 = np.copy(r_values_a0)
        self._r_values_a4 = np.copy(r_values_a4)
        self._rho_core_values_a0 = np.copy(rho_core_values_a0)
        self._rho_core_values_a4 = np.copy(rho_core_values_a4)
        self._N1_array = np.copy(N1_array)
        self._N2_array = np.copy(N2_array)

        return
    
class Hard_SIDM_Version2():

    # def __init__(self, density_3d_array, rvals_array, N1_array, N2_array, sepa_init=1.0e3*PC, num_steps=300):
    def __init__(self, density_3d_array, rvals_array, N1_array, N2_array, sepa_init=1.0*PC, num_steps=300):
        self._num_steps = num_steps
        self._sepa_init = sepa_init
        # Ensure the arrays are writable by creating copies
        self._density_3d_array = np.copy(density_3d_array)
        self._r_values = np.copy(rvals_array)
        self._N1_array = np.copy(N1_array)
        self._N2_array = np.copy(N2_array)
...
class Hard_3BS():
    ...
    def __init__(self, rho_i_1d_array, sigma_i_1d_array, H, sepa_init=1.0e3*PC, num_steps=300):
        self._num_steps = num_steps
        self._sepa_init = sepa_init
        # Ensure the arrays are writable by creating copies
        self._rho_i_1d_array = np.copy(rho_i_1d_array)
        self._sigma_i_1d_array = np.copy(sigma_i_1d_array)
        self._H = H
