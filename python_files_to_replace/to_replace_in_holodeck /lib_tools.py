...
def run_model(
    ...
    if not isinstance(hard, (holo.hardening.Fixed_Time_2PL_SAM, 
                             ...
                             holo.hardening.Hard_CDM, 
                             holo.hardening.Hard_SIDM, 
                             holo.hardening.Hard_3BS, 
                             holo.hardening.Hard_SIDM_Version2)):
        err = f"`holo.hardening.Fixed_Time_2PL_SAM` must be used here!  Not {hard}!"
        if log is not None:
            log.exception(err)
        raise RuntimeError(err)
    print(f'{datetime.now().strftime("%H:%M:%S")}: starting calculating redz_final, diff_num')
    redz_final, diff_num = sam_cyutils.dynamic_binary_number_at_fobs(
        fobs_orb_cents, sam, hard, cosmo
    )
    print(f'{datetime.now().strftime("%H:%M:%S")}: redz_final, diff_num done')
    use_redz = redz_final
    edges = [sam.mtot, sam.mrat, sam.redz, fobs_orb_edges]
    print(f'{datetime.now().strftime("%H:%M:%S")}: starting number calculation')
    number = sam_cyutils.integrate_differential_number_3dx1d(edges, diff_num)
    print(f'{datetime.now().strftime("%H:%M:%S")}: number calculation done')
    print(f'details_flag:{details_flag}, singles_flag:{singles_flag}, params_flag:{params_flag}')
    if details_flag:
    ...
    if singles_flag or params_flag:
        ...
        if singles_flag:
            data['hc_ss'] = hc_ss
            data['hc_bg'] = hc_bg
    print(f'{datetime.now().strftime("%H:%M:%S")}: starting calculating gwb') # for checks
    if gwb_flag:
        ##### aded by Shreyas (09.12.2025) to fix holodeck's redz=-1 issue that creeps in 4d redz midpoint calculations
        negative_mask = use_redz < 0.0
        use_redz[negative_mask] = -np.inf
        ################################################
        gwb = holo.gravwaves._gws_from_number_grid_integrated_redz(edges, use_redz, number, nreals)
        data['gwb'] = gwb
    print(f'{datetime.now().strftime("%H:%M:%S")}: gwb done')
    return data
