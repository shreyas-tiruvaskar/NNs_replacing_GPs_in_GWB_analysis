...
class ceffylGP():
    ...
    def __init__(self, datadir, hyperparams, gp, gp_george, var_gp,
                 ...
                 nn_dir=None, # added by Shreyas (24.11.2025)
                 ):
        import numpy as np
        """
        ...
        # added by Shreyas (23.02.2026) to measure nn and gp prediction time
        import time
        self.interp_time = 0.0
        self.n_calls = 0
        ####################################################################
        # added by Shreyas (25.11.2025)
        if nn_dir is not None:
            ...
            self.model_median = keras.models.load_model(
            ...
            self.model_std = keras.models.load_model(
            ...
            self.mu_x = normalization_params["mu_x"]
            self.std_x = normalization_params["std_x"]
            self.mu_y_median = normalization_params["mu_y_median"]
            self.std_y_median = normalization_params["std_y_median"]
            self.std_y_std = normalization_params["std_y_std"]
    ...
    def ln_likelihood(self, x0):
        ...
        # return 0.0  # return 0, added by Shreyas, to get only priors
    
    ################################# added by Shreyas (11.11.2025)
    def ln_likelihood_all_freqs(self, x0):
        """
        likelihood function for all frequencies separately
        """
        # ensure constant values are in the correct place!
        etac = np.zeros(len(self.hyperparams))  # empty array
        etac[self.hypervar_idx] = x0
        ...
        self.interp_time += (t1 - t0)
        self.n_calls += 1
        print("Total interpolator time:", self.interp_time)
        print("Calls:", self.n_calls)
        print("Time per prediction:", self.interp_time / self.n_calls)
        ####################################################################
        ...
        return ln_like  # return ln gaussian for all frequencies: shape(5,)
    ###############################################################################
    
    ################################# added by Shreyas (24.11.2025)
    def ln_likelihood_nn(self, x0):
        ...
        # unnormalized_test_param = etac.reshape(1, -1)
        # normalized_test_param = (unnormalized_test_param - self.mu_x) / self.std_x
        # normalized_test_param_tf = tf.convert_to_tensor(normalized_test_param, dtype=tf.float32)
        # unnormalized_median_prediction = self.model_median(normalized_test_param_tf, training=False)
        # normalized_median_prediction = unnormalized_median_prediction * self.std_y_median + self.mu_y_median # log10hc2
        # hc = (10**(normalized_median_prediction/2)).numpy().reshape((5,))
        # unnormalized_std_prediction = self.model_std(normalized_test_param_tf, training=False)
        # normalized_std_prediction = unnormalized_std_prediction * self.std_y_std + self.mu_y_std
        # log10h2cf_sigma = normalized_std_prediction.numpy().reshape((5,))

        unnormalized_test_param = etac.reshape(1, len(etac)) # shape (1, 6)
        normalized_test_param = (unnormalized_test_param - self.mu_x) / self.std_x
        y_pred_dist_median = self.model_median(normalized_test_param)
        y_pred_median_nn = y_pred_dist_median.mean()
        y_pred_median_unc_nn = y_pred_dist_median.stddev()
        y_pred_dist_std = self.model_std(normalized_test_param)
        ...
        y_pred_total_original_std_nn = y_pred_total_std_nn * self.std_y_std
        y_pred_original_median_nn = y_pred_median_nn * self.std_y_median + self.mu_y_median
        hc = 10**(y_pred_original_median_nn.numpy().reshape(5,) / 2)
        log10h2cf_sigma = y_pred_total_original_std_nn.reshape(5,)
        ...
        return np.sum(ln_like)  # return ln gaussian for all frequencies: shape(5,)
    
    def ln_likelihood_nn_all_freqs(self, x0):
        # import tensorflow as tf
        # import numpy as np
        # import tensorflow_probability as tfp
        # tfpl = tfp.layers
    ...
    def holospectrum_lnlikelihood_all_freqs(self, x0):
        """
        function to fit holodeck simulations quickly
        w/o GPs. Not as accurate, but rather fast!
        """
...
class ceffylGPSampler():
    ...
    def __init__(self, trainedGP, trained_varGP, ceffyldir,
        ...
        @param test_sigma: hard coded std dev to add test powerlaw at each freq
                           if test==True
        """
        import numpy as np # added by Shreyas (25.11.2025)
        import h5py # added by Shreyas (25.11.2025)
        import pickle # added by Shreyas (25.11.2025)
        ...
        ceffyl_gp = ceffylGP(ceffyldir, Nfreqs=Nfreqs, hyperparams=hyperparams,
                             ...
                             spectrum=spectra,
                             nn_dir=nn_dir, # added by Shreyas (24.11.2025)
                             )
        ...
        if analysis_type == 'test':  # flag to test code against a powerlaw w/ variance
            ...
            logl_all_freqs = ceffyl_gp.holospectrum_lnlikelihood_all_freqs  # added by Shreyas (05.01.2026)
        elif analysis_type == 'gp':
            logl = ceffyl_gp.ln_likelihood
            logl_all_freqs = ceffyl_gp.ln_likelihood_all_freqs  # added by Shreyas (11.11.2025)
        # added by Shreyas (24.11.2025)###############################################
        elif analysis_type == 'nn':
            if nn_dir is None:
                raise ValueError("Please provide nn_dir for 'nn' analysis_type")
            else:
                logl = ceffyl_gp.ln_likelihood_nn  # added by Shreyas (24.11.2025)
                logl_all_freqs = ceffyl_gp.ln_likelihood_nn_all_freqs  # added by Shreyas (24.11.2025)
        ###############################################################################
            
        else:
            print("Please choose between 'test', 'holo_spectra', 'gp', and 'nn' \n")
            return
        
        # sampler = ptmcmc(len(x0), logl=logl,
        sampler = ptmcmc(len(x0), logl=logl, logl_all_freqs=logl_all_freqs,  # added by Shreyas (11.11.2025)
