...
n_training = 8000
OUTPUT = f'{PROJECT_ROOT}/holodeck_output_lib_gp_mcmc/SIDM_Astro_Uniform_Extended_Version2_n{n_training}_r2000_f5/library/'
...
outdir = str(
    Path(OUTPUT).parent
    / 'gp_mcmc_log_likelihood_all_freqs'
...
for hp in hp_names:
    if (hp=='sigma0_over_m_times_t_age_per_1Gyr') or (hp=='vt'):
        h = parameter.Uniform(gp[0].par_dict[hp]['min'],
                            gp[0].par_dict[hp]['max'])(hp)
