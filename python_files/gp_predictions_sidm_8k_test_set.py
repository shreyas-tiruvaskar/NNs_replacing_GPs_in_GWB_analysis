...
## SIDM ##
# GP predictions for test set
N_test = 8000
npz_path = f'{PROJECT_ROOT}/holodeck_output_lib_gp_mcmc/SIDM_Astro_Uniform_Extended_Version2_n{N_test}_r2000_f5/library2_gp_nn_accuracy/gp_predictions_for_{N_test}_test_points.npz'
hdf5_file_path = f'{PROJECT_ROOT}/holodeck_output_lib_gp_mcmc/SIDM_Astro_Uniform_Extended_Version2_n{N_test}_r2000_f5/library2_gp_nn_accuracy/sam-library.hdf5'

gp_dir = Path(f'{PROJECT_ROOT}/holodeck_output_lib_gp_mcmc/SIDM_Astro_Uniform_Extended_Version2_n{N_test}_r2000_f5/gp_training_output')
###########
...
with tqdm_joblib(tqdm(total=param_space_test.shape[0])) as progress_bar:
    results = Parallel(n_jobs=N_cores)(
        delayed(run_gp)(i) for i in range(param_space_test.shape[0])
    )

pred_med_log10_hc2_array = np.zeros((param_space_test.shape[0], NFREQS))
pred_std_log10_hc2_array = np.zeros((param_space_test.shape[0], NFREQS))
